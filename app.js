...
let pvr = [
    {
        ...
        video: 'video/From the World of John Wick_ Ballerina (2025) Official Trailer - Ana de Armas.mp4'
    },
    {
        pvr: 'PVR Royal',
        movie: 'Nobody2',
        ...
        date: 18,
        img: 'img/Nobody2.jpg',
        video: 'video/Nobody 2 _ Official Trailer.mp4'
    },
    {
        pvr: 'PVR Royal',
        movie: 'Ballerina',
...
console.log("main_date:", main_date);
console.log("movie from URL:", url_segment[1]);

let data = pvr.filter(obj => 
    obj.date == main_date && obj.movie.trim() == url_segment[1].trim()
);
// console.log(data);
...
document.getElementById('book_ticket').addEventListener('click', () => {
    ...
    selectedSeats.forEach(el => {
        let seat_no = +el.getAttribute('book');      // seat number
        let seat_sr = el.getAttribute('sr');         // seat row (A, B, ...)

        // Update pvr data in memory
        pvr = pvr.map((obj) => {
            if (obj.movie.trim() === url_segment[1].trim() && obj.date === main_date) {
                if (!obj[seat_sr.toLowerCase()].includes(seat_no)) {
                    obj[seat_sr.toLowerCase()].push(seat_no);
                }
            }
            return obj;
        });
        ...
        tic.innerHTML = `
            ...
            <div class="tic_details">
                ...
                <h6 class="pvr"><span>Royal</span> Cinema</h6>
                <h1>${url_segment[1].trim()}</h1>
                <div class="seat_det">
                    <div class="seat_cr">
                        <h6>ROW</h6>
        ...
        // Generate barcode
        JsBarcode(`#${seat_sr}${seat_no}barcode`, `${seat_sr.toUpperCase()}${seat_no}${main_date}2025${el.innerText}`);
    });

    // Hide seats and show tickets
...
document.getElementById('back_ticket').addEventListener('click', () => {
    ...
    // Re-render seats
    document.getElementById('chair').innerHTML = '';
    let data = pvr.filter(obj => obj.date === main_date && obj.movie.trim() === url_segment[1].trim());
    addSeats(data);
});
